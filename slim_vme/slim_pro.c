...
int ispVMShiftExec(unsigned int a_uiDataSize);
int ispVMShift(char a_cCommand);
unsigned char GetByte(int a_iCurrentIndex, char a_cAlgo);
void ispVMStateMachine(unsigned char a_cNextState);
void ispVMClocks(unsigned int a_usClocks);
void ispVMBypass(unsigned int a_siLength);
void sclock();
int ispVMRead(unsigned int a_uiDataSize);
void ispVMSend(unsigned int a_uiDataSize);
void ispVMLCOUNT(unsigned int a_usCountSize);
void ispVMLDELAY();
/*************************************************************
*                                                            *
* EXTERNAL FUNCTION                                          *
...
unsigned int g_usDataType = 0x0000; /*** data type register used to hold information ***
...
int g_siHeadDR = 0;     /*** used to hold the header data register ***/
int g_siHeadIR = 0;     /*** used to hold the header instruction register ***/
int g_siTailDR = 0;     /*** used to hold the trailer data register ***/
int g_siTailIR = 0;     /*** used to hold the trailer instruction register ***/
...
unsigned char g_siIspPins = 0x00;   /*** holds the current byte to be sent to the hardware ***/
char g_cCurrentJTAGState = 0;   /*** holds the current state of JTAG state machine ***/

unsigned int  g_iLoopIndex = 0;
int  g_iLoopMovingIndex = 0;    /*** Used to point to the location of LOOP data ***/
int  g_iLoopDataMovingIndex = 0;

unsigned int g_usLCOUNTSize = 0;
unsigned char  g_ucLDELAYState = IDLE;
unsigned int  g_ucLDELAYTCK = 0;
unsigned int  g_ucLDELAYDelay = 0;
unsigned int m_loopState = 0;
            ...
            *************************************************************/
            ...
            case LOOP:
                ...
                *************************************************************/
                ...
                g_usLCOUNTSize = ispVMDataSize();
                //printf( "MaxLoopCount %d\n", g_usLCOUNTSize );
                //DPRINT(LOG_DEBUG,"%s: MaxLoopCount %d\r\n", __func__,g_usLCOUNTSize);
...
unsigned char GetByte(int a_iCurrentIndex, char a_cAlgo)
{
    ...
    if (a_cAlgo)
    {
        ...
        *************************************************************/
        ...
        //DPRINT(LOG_DEBUG,"%s: algo[%d]=0x%X\r\n", __func__,a_iCurrentIndex, ucData);
        //delay_ms(100);
    }
    else
    {
        ...
        *************************************************************/
        ...
        //DPRINT(LOG_DEBUG,"%s: data[%d]=0x%X\r\n", __func__,a_iCurrentIndex, ucData);
        //delay_ms(100);
    }
...
void ispVMStateMachine(unsigned char a_cNextState)
{
    unsigned char cPathIndex, cStateIndex;
...
void ispVMLDELAY()
{
    ...
    while (1)
    {
        ...
        case WAIT:  /*opcode to wait for specified time in us or ms*/
            g_ucLDELAYDelay = ispVMDataSize();
            break;
        case TCK:   /*pulse TCK signal the specified time*/
            g_ucLDELAYTCK = ispVMDataSize();
            break;
        case ENDSTATE:
            return;
            break;
